...
class Welcome extends React.Component<ElementProps<View> & { color: string }> {
    ...
    testNativeMethods() {
        if (this.rootViewRef.current != null) {
            ...
            this.rootViewRef.current.measure((x: number, y: number, width: number, height: number) => {});
        }
    }
    ...
    render() {
        ...
        return (
            <View {...props} ref={this.rootViewRef} style={[[styles.container], undefined, null, false]}>
    ...
    render() {
        return (
            <>
                ...
                <TouchableOpacity aria-hidden={true} aria-label="Aria Label" aria-busy aria-valuemax={1} />
                <TouchableNativeFeedback aria-expanded={false} aria-label="Aria Label" />
            </>
        );
    }
}
...
const NativeBridgedComponent = requireNativeComponent<{ nativeProp: string }>('NativeBridgedComponent');
